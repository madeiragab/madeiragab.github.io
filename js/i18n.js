...
const TEXTOS = {
  ...
  pt: {
    ...
    'sobre.p3': 'Também gosto de sair da zona de conforto: escrevi um banco de dados do zero em Rust — pager, B+Tree e WAL com recuperação de crash —, um sistema que mede se dá para prever a bolsa (e conclui que quase não dá), jogo por turnos em Pygame, simulador de IA em Godot pro TCC e engenharia reversa de um console portátil clone. Entender como as coisas <span class="red">quebram</span> é a melhor forma de aprender a construí-las melhor.',
    ...
    'proj.titulo': 'ONDE BACKEND, SISTEMAS, DADOS E HARDWARE SE ENCONTRAM.',
    ...
    'skill.rust': '▸ Rust / Sistemas',
    'skill.onde.rust': '<a href="https://github.com/madeiragab/lastro">Lastro</a> — banco de dados relacional embutido escrito do zero: pager com cache, B+Tree, WAL com recuperação de crash, parser SQL e MVCC. Sem ORM e sem SQLite embaixo.',
    'skill.dados': '▸ Dados / Machine Learning',
    'skill.onde.dados': '<a href="https://github.com/madeiragab/market-direction">market-direction</a> — previsão direcional de ações com pandas, scikit-learn e LightGBM. Validação walk-forward com embargo, auditoria de vazamento que roda como teste e custo de transação no backtest.',
    'skill.hardware': '▸ Linux / Hardware',
    'skill.onde.python': 'Backend do <a href="https://github.com/madeiragab/rastro">Rastro</a> (FastAPI) e do <a href="https://github.com/madeiragab/geicis-ponto">GEICIS Ponto</a> (Django); simulador do <a href="https://github.com/madeiragab/tcc-simulador-ia">TCC</a>, com +14.000 partidas medidas; pipeline de dados e backtest do <a href="https://github.com/madeiragab/market-direction">market-direction</a>.',
  ...
  en: {
    ...
    'sobre.p3': 'I also like leaving my comfort zone: I wrote a database from scratch in Rust — pager, B+Tree and a WAL with crash recovery —, a system that measures whether the stock market can be predicted (and concludes it mostly cannot), a turn-based game in Pygame, an AI simulator in Godot for my thesis, and reverse-engineered a clone handheld console. Understanding how things <span class="red">break</span> is the best way to learn how to build them better.',
    ...
    'proj.titulo': 'WHERE BACKEND, SYSTEMS, DATA AND HARDWARE MEET.',
    ...
    'skill.rust': '▸ Rust / Systems',
    'skill.onde.rust': '<a href="https://github.com/madeiragab/lastro">Lastro</a> — an embedded relational database written from scratch: cached pager, B+Tree, WAL with crash recovery, SQL parser and MVCC. No ORM and no SQLite underneath.',
    'skill.dados': '▸ Data / Machine Learning',
    'skill.onde.dados': '<a href="https://github.com/madeiragab/market-direction">market-direction</a> — directional stock prediction with pandas, scikit-learn and LightGBM. Walk-forward validation with an embargo, a leakage audit that runs as a test, and transaction cost inside the backtest.',
    'skill.hardware': '▸ Linux / Hardware',
    'skill.onde.python': 'Backend of <a href="https://github.com/madeiragab/rastro">Rastro</a> (FastAPI) and <a href="https://github.com/madeiragab/geicis-ponto">GEICIS Ponto</a> (Django); the <a href="https://github.com/madeiragab/tcc-simulador-ia">thesis</a> simulator, with 14,000+ measured matches; the data pipeline and backtest in <a href="https://github.com/madeiragab/market-direction">market-direction</a>.',
  ...
  es: {
    ...
    'sobre.p3': 'También me gusta salir de la zona de confort: escribí una base de datos desde cero en Rust — pager, B+Tree y WAL con recuperación de fallos —, un sistema que mide si se puede predecir la bolsa (y concluye que casi no), un juego por turnos en Pygame, un simulador de IA en Godot para la tesis e ingeniería inversa de una consola portátil clon. Entender cómo las cosas <span class="red">se rompen</span> es la mejor forma de aprender a construirlas mejor.',
    ...
    'proj.titulo': 'DONDE BACKEND, SISTEMAS, DATOS Y HARDWARE SE ENCUENTRAN.',
    ...
    'skill.apis': '▸ APIs REST',
    'skill.rust': '▸ Rust / Sistemas',
    'skill.onde.rust': '<a href="https://github.com/madeiragab/lastro">Lastro</a> — base de datos relacional embebida escrita desde cero: pager con caché, B+Tree, WAL con recuperación de fallos, parser SQL y MVCC. Sin ORM y sin SQLite debajo.',
    'skill.dados': '▸ Datos / Machine Learning',
    'skill.onde.dados': '<a href="https://github.com/madeiragab/market-direction">market-direction</a> — predicción direccional de acciones con pandas, scikit-learn y LightGBM. Validación walk-forward con embargo, auditoría de fuga que corre como test y coste de transacción dentro del backtest.',
    'skill.hardware': '▸ Linux / Hardware',
    'skill.onde.python': 'Backend de <a href="https://github.com/madeiragab/rastro">Rastro</a> (FastAPI) y de <a href="https://github.com/madeiragab/geicis-ponto">GEICIS Ponto</a> (Django); simulador del <a href="https://github.com/madeiragab/tcc-simulador-ia">TFG</a>, con +14.000 partidas medidas; pipeline de datos y backtest de <a href="https://github.com/madeiragab/market-direction">market-direction</a>.',
...
const DESCRICOES = {
  pt: {
    'market-direction': 'Previsão direcional de ações para B3 e S&P 500, construída em torno de medir se dá para acreditar na previsão: walk-forward com embargo, auditoria de vazamento e custo de transação. O resultado honesto é que não bate comprar e segurar.',
    'lastro': 'Banco de dados relacional embutido escrito do zero em Rust: pager, B+Tree, WAL com recuperação de crash, parser SQL e MVCC.',
    'ascensao-dos-semideuses': 'RPG de mesa autoral: semideuses gregos no mundo moderno. Livro do Jogador, Bestiário e simulador de balanceamento em Python com 40 mil combates medidos.',
    'rastro': 'Rastreamento em tempo real e geocerca para rebanho bovino. FastAPI + PostGIS, autenticação Argon2id com rotação de refresh token e simulador de rebanho embutido.',
    'geicis-ponto': 'Sistema de ponto para estagiários do GEICIS. Domínio modelado antes do código: Django + DRF, ranking de horas e alerta semanal por e-mail.',
    'social-network': 'Rede social backend-first: domínio modelado em UML antes do código, regras de negócio no servidor e documentação completa.',
    'rpg-panel': 'Painel web para gerenciar campanhas de RPG de mesa — campanhas, personagens, inventário e papéis de mestre/jogador.',
    'tcc-simulador-ia': 'Simulador tático (TCC) para avaliar a qualidade estratégica de agentes de IA em ambientes de decisão.',
    'Guns-and-boots': 'Jogo 2D retrô-futurista por turnos, com máquina de estados, sistema de save e modo mobile.',
    'darkos-ga36-port': 'Autópsia, preservação e documentação de um console portátil clone (GA36-MB / Allwinner A33).',
  },
  en: {
    'market-direction': 'Directional stock prediction for B3 and the S&P 500, built around measuring whether the prediction is worth believing: walk-forward with an embargo, a leakage audit and transaction cost. The honest result is that it does not beat buy and hold.',
    'lastro': 'Embedded relational database written from scratch in Rust: pager, B+Tree, WAL with crash recovery, SQL parser and MVCC.',
    'ascensao-dos-semideuses': 'Original tabletop RPG: Greek demigods in the modern world. Player handbook, bestiary and a Python balancing simulator with 40,000 measured fights.',
    'rastro': 'Real-time tracking and geofencing for cattle herds. FastAPI + PostGIS, Argon2id auth with refresh-token rotation and a built-in herd simulator.',
    'geicis-ponto': 'Time-tracking system for GEICIS interns. Domain modeled before the code: Django + DRF, hour ranking and a weekly e-mail alert.',
  ...
  es: {
    'market-direction': 'Prediccion direccional de acciones para B3 y S&P 500, construida en torno a medir si la prediccion merece credito: walk-forward con embargo, auditoria de fuga y coste de transaccion. El resultado honesto es que no supera comprar y mantener.',
    'lastro': 'Base de datos relacional embebida escrita desde cero en Rust: pager, B+Tree, WAL con recuperacion de fallos, parser SQL y MVCC.',
    'ascensao-dos-semideuses': 'Juego de rol de mesa propio: semidioses griegos en el mundo moderno. Manual del jugador, bestiario y simulador de balance en Python con 40.000 combates medidos.',
    'rastro': 'Rastreo en tiempo real y geocerca para ganado bovino. FastAPI + PostGIS, autenticacion Argon2id con rotacion de refresh token y simulador de rebano integrado.',
    'geicis-ponto': 'Sistema de fichaje para becarios del GEICIS. Dominio modelado antes del codigo: Django + DRF, ranking de horas y alerta semanal por correo.',
